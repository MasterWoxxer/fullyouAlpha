...
interface RelationshipScaleProps {
  scales: AssessmentScale[];
  onScaleChange: (index: number, newValue: number) => void;
}

const RelationshipScale: React.FC<RelationshipScaleProps> = ({ scales, onScaleChange }) => {
  ...
  return (
    <div className="mb-8">
      {scales.map((scale, index) => (
        <div key={index} className="mb-8 scale-container">
          ...
          <div className="flex items-center gap-3">
            <span className="w-40 text-right text-sm pr-1 text-gray-600 dark:text-gray-400">{scale.leftLabel}</span>
            <div 
            ...
            <span className="w-40 text-left text-sm pl-1 text-gray-600 dark:text-gray-400">{scale.rightLabel}</span>
          </div>
        </div>
      ))}
    </div>
...
export default RelationshipScale;
